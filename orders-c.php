			...
			<div class="wrapContent">
				<div class="allData" style="width: 100%;">
					<div class="otherLinks">
						<span onclick="window.location='orders.php';">Pending</span> | 
						<span onclick="window.location='orders-c.php';" class="activePageLink">Confirmed</span> | 
						<span onclick="window.location='orders-r.php';">Rejected</span> |
						<span onclick="window.location='orders-ca.php';">Cancelled</span>
					</div>
					<div class="heading">All received orders data(Confirmed)</div>
					<table>
						<tr>
							<th></th>
							<th>Order Code</th>
						...
						<?php
							...
							$query = mysqli_query($connect,"SELECT * FROM orders WHERE status = 'Confirmed' ORDER BY ordercode DESC");
							$rowcount = 0;
							while ($row = mysqli_fetch_array($query)){
								...
								echo "<tr>
									...
									<td>".$row['status']."</td>
									<td>
										<a href='server.php?cancelOrder=".$row['id']."&product=".$row['productid']."&qty=".$row['qty']."'><button class='denybtn'>Order Cancellation</button></a>
									</td>
								</tr>";
							}
						?>
